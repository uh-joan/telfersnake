...
export class CooperView {
  ...
  constructor() {
    ...
    for (const side of [-1, 1]) {
      ...
      leg.add(box(0.14, 0.9, 0.16, SUIT, 0, -0.45, 0), box(0.15, 0.09, 0.3, SHOE, 0, -0.92, 0.06));
      this.legs.push(leg);

      const arm = new THREE.Group();
      arm.position.set(side * 0.27, 1.55, 0);
      arm.add(box(0.1, 0.58, 0.12, SUIT, 0, -0.29, 0), box(0.09, 0.1, 0.1, SKIN, 0, -0.63, 0));
      this.arms.push(arm);
      f.add(leg, arm);
    }

    f.add(
      box(0.42, 0.66, 0.22, SUIT, 0, 1.27, 0), // jacket
      box(0.13, 0.42, 0.02, SHIRT, 0, 1.37, 0.115),
      box(0.055, 0.36, 0.02, TIE, 0, 1.36, 0.13),
    ...
    const hair = new THREE.Mesh(new THREE.SphereGeometry(0.142, 12, 8, 0, Math.PI * 2, 0, Math.PI * 0.52), mat(HAIR));
    ...
    for (const side of [-1, 1]) {
      ...
      const brow = box(0.05, 0.012, 0.02, HAIR, side * 0.05, 1.86, 0.125);
      f.add(eye, brow);
    }
