...
const brick = (x: number, y: number, z: number, turn: number): Geo =>
  paint(new THREE.BoxGeometry(0.5, 0.16, 0.24), 0xb5482f, (g) => g.rotateY(turn).translate(x, y, z));
...
const MODELS: Record<HazardKind, () => Geo[]> = {
  ...
  bricks: () => [brick(-0.1, 0.08, 0.05, 0.2), brick(0.12, 0.24, -0.02, -0.5), brick(0.32, 0.08, 0.34, 1.2)],
};
