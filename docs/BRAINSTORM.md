...
- **Difficulty modes (done, 2026-09-21):** three ways to play, chosen on the start screen. A mode is a small `Rules` bundle (`src/sim/modes.ts`) handed to the `World` — rival personalities, food count, and a `botsGetUpgrades` flag — so the whole engine is reused.
  - **🐣 Easy** — rivals shrunk (mass cap ×0.5), slowed (≤0.8), made timid and oblivious (no hunting, no dashing), and a bit more food (55): a five-year-old is soon the biggest snake in the yard. It is the **default for new players** (remembered once changed) and pays **half stars** (`MODE_STARS` in `main.ts`), so it is a gentle sandbox, not a star farm — Normal is the road to anything dear.
  - **⚡ Normal** — exactly the tuning the game shipped with (rivals unchanged, food 42, full stars). The reference.
  - **🧙 God** — rivals faster than a fresh player (≤1.15), greedier and cannier, and they **take level-up cards too** (the one `if (bot) s.pendingCards = 0` in `world.step()` is flipped; bots auto-grab the scariest card via `botCardChoice`, never freezing). The secret challenge, and it must be *earned in the real game*: hidden until you own **both** the Golden Snake skin (500★) and the Wizard Hat (2000★) **and** have gone MEGA (the top size tier) in a **Normal** game (the sticky `save.mega` flag, set from the player's tier event) — so it can't be bought purely by farming Easy. It then reveals itself once with a little flourish. `godUnlocked(owned, mega)` gates it at load, in the picker, and in `save.ts` against hand-edited saves. Easy and Normal are always available.
...
- **Blue gems + Powers (done, 2026-09-21):** a second currency and an offensive-upgrade family, built on the existing sim/card/protocol rails.
  - **Powers** (`POWER_IDS`: `laser`, `stink`, `zap`) are epic upgrades that auto-fire on a cooldown (no aiming) and shrink rivals via the same `shed()` a rock uses — 👁️ Laser Eyes (a forward beam, first rival in line), 💨 Stink Cloud (a puff behind you, shrinks chasers), ⚡ Zap Ring (a 360° shock, everyone close). New `power` (cast) and `hit` (victim) events drive the FX and flow through the existing snapshot event channel. Fire Breath (`dragon`, a base upgrade) was reworked earlier to scorch rivals the same way.
  - **Blue gems** (`save.gems`): earned 1 per rival you bonk or scorch/shrink (fire breath included), banked like stars (multi-tab-safe delta), shown on the results screen and in the shop.
  - **Unlock loop**: spend gems in a new Tuck Shop **Powers** tab to unlock a power once (`save.powers`); unlocked powers then join your level-up card pool (`rollCards` gates on the per-snake `powers` set; base upgrades always roll). Two currencies with clearly different jobs/colours — 🟡 dress-up vs 🔵 zappy powers.
  - **Multiplayer-aware**: the unlocked set rides the `hello` message; the server validates it and seats the player with it. God bots get the whole arsenal (`seatBot`; `botCardChoice` prefers powers) so they fight back with lasers and zaps. Powers persist across respawns (they're unlocks, not in-run upgrades); only their in-run *level* resets on a bonk, like any upgrade. Kept cartoon and gentle: rivals shrink & puff, nobody's hurt.
